...
class Api @Inject constructor(
    ...
    suspend fun getPlaylistSongs(playlistId: String): Flow<Response<List<SongsModel>>> = flow {
        ...
        if (playlistId.startsWith("youtube_api_") || playlistId.startsWith("imported_")) {
            val imported = com.music.spotui.data.preferences.OfflineCollectionsPref.getCollection(context, playlistId)
            if (imported != null) {
                emit(Response.Success(imported.songs))
                return@flow
            }
        }
        if (!SpotifyTokenProvider.ensureToken(context)) {
    ...
    suspend fun getPlaylist(playlistId: String): Flow<Response<AlbumsModel>> = flow {
        ...
        if (playlistId.startsWith("youtube_api_") || playlistId.startsWith("imported_")) {
            ...
            if (imported != null) {
                emit(Response.Success(AlbumsModel(
                    id = stableId("playlist:${imported.id}"),
                    artists = imported.artists.ifBlank { if (imported.id.startsWith("youtube_api_")) "YouTube Music" else "Imported collection" },
                    coverUri = imported.coverUri,
                    name = imported.name,
                    time = "${imported.songs.size} song" + if (imported.songs.size == 1) "" else "s",
                )))
                return@flow
            }
        }
